...
登录页密码框默认隐藏。浏览器启用 JavaScript 时，点击「显示密码」或用 Tab 聚焦后按
Enter / Space 可切换显示状态；切换不会提交表单、改变 URL 或触达认证服务。禁用 JavaScript
时，原生密码输入与「登录」提交按钮仍可正常使用。切换状态仅存在于当前页面，不写入本地存储。
