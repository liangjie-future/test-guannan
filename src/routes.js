 ...
 */
export function createRoutes({ getTimeline, timelineUsesSessionToken = false } = {}) {
  const timelinePage = createTimelinePage({ getTimeline, useSessionToken: timelineUsesSessionToken });
  ...
  <p>导航登录态由 currentUser 注入点提供（FP-003 会话管理联调；未合入时默认未登录）。</p>
    ...
    '/timeline': {
      title: '时间线',
      render: ({ user, sessionToken }) =>
        user ? timelinePage.render(user, sessionToken) : timelinePage.renderAnonymous(),
    },
    '/logout': {
      title: '退出',
